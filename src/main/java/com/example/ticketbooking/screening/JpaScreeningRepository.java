...
public interface JpaScreeningRepository extends ScreeningRepository, JpaRepository<Screening, Integer> {
    ...
    @Override
    @Query("Select new com.example.ticketbooking.screening.ScreeningWithMovieTitleDTO(s.movie.title, s.movieDate, s.movieStartTime) from Screening s join s.movie where s.movieDate = :date")
    List<ScreeningWithMovieTitleDTO> findByDate(@Param("date") LocalDate date);

    @Override
    @Query("Select s from Screening s join fetch s.movie where s.movieDate = :date and s.movieStartTime between :startTime and :endTime ORDER BY s.movieStartTime, s.movie.title")
